...
# This program generates a file containing the next neighbors up to nth order relative to
# one fixed atom with the isotropic Heisenberg exchange paramters
...
# isotropic Heisenberg exchange for bcc iron taken from https://journals.aps.org/prb/pdf/10.1103/PhysRevB.82.144304
...
for n, d in enumerate(sorted_dict):
    if n >= len(J_n):
        break
    for v in dict[d]:
        file.write(str(v[0]) + "," + str(v[1]) + "," + str(v[2]) + "," + str(J_n[n]) + "\n")
