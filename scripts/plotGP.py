...
eigenenergies_path = 'Outputs/GP_path/eigenenergies_GP.txt'
ph_dispersion_path = 'Outputs/GP_path/GP_phdisp.txt'

eigenenergies = retrieve_data_from_file(eigenenergies_path)

phdisp = retrieve_data_from_file_space(ph_dispersion_path)

magdisp = retrieve_data_from_file('Outputs/GP_path/mag.txt')
...
axin = inset.inset_axes([0.6, 0.12, 0.35, 0.78])
...
axin.set_xlim(0.04,0.08)
axin.set_ylim(0.9,2.95)
...
axs[0].set_xticks([0,1], labels=[r'$\Gamma$', 'P'])
...
plt.savefig('scripts/Figures/GP_path.png', dpi=600)
plt.show()
