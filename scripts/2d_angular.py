...
data_ev = "Outputs/colpa_grid/ang_eig_fromEV.txt"
data_phonon = "Outputs/colpa_grid/grid_acc2_formatted.txt"
...
with open(data_phonon, 'r') as f:
    ...
    for line_count, line in enumerate(f):
        if line_count in skipped_lines:
            continue
        if line == "\n":
            continue
        line = line.strip()
...
plt.tight_layout()
plt.savefig("scripts/Figures/ang_eig_fromEV_acc_colpa.png", dpi=500)
plt.show()
plt.clf()
...
cbar.set_ticks([-1,0,1])
plt.tight_layout()
plt.savefig("scripts/Figures/ang_eig_fromEV_acc_3d_colpa.png", dpi=700)
plt.show()
plt.clf()
