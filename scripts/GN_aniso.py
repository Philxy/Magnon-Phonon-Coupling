...
with open("Outputs/GN_anisotropy/eigenenergies.txt", "r") as file:
    line = file.readline()
    for line in file:
        line = line.strip()
        line = line.split(",")
...
with open("Outputs/GN_dmi/eigenenergies.txt", "r") as file:
    line = file.readline()
    for line in file:
        line = line.strip()
        line = line.split(",")
...
with open("Outputs/GN_all_inter/eigenenergies.txt", "r") as file:
    line = file.readline()
    for line in file:
        line = line.strip()
        line = line.split(",")
...
axs[1].set_xlim(0.045, 0.1)
axs[1].set_ylim(0.9, 3.5)
...
axs[0].set_xticklabels([r'$\Gamma$', r'$N$'])
axs[1].legend()
axs[0].set_ylabel('dispersion relation (meV)')
plt.tight_layout()
plt.savefig("scripts/Figures/GNaniso.png", dpi=500)
plt.show() 
plt.clf()
