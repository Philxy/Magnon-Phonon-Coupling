...
S_inv_matrices = [np.linalg.inv(
    np.array(S_matrix[i]).reshape(8, 8).transpose()) for i in range(len(S_matrix))]
S_matrices = [np.array(S_matrix[i]).reshape(8, 8).transpose()
              for i in range(len(S_matrix))]

wilket = int(0.05*1000)
...
for i in range(8):
    ...
    for j in range(8):
        ...
        row.append(f"{real:.2f}+{imag:.2f}j")
