...
void SamplingGrid::initMagnonDisp(std::string couplingParameterFile, double S)
{
    ...
    std::vector<MagnonDispParam> magDisp = getMagneticDispersion(couplingParameterFile, path, S);
...
void SamplingGrid::initPhononDisp(std::string dynamicMatricesFile, std::string nextNeighbourFile, double atomic_mass)
{
    std::vector<Vector3D> path;
    ...
    std::vector<PhononDispParam> phDisp = getPhononDispersion(dynamicMatricesFile, nextNeighbourFile, path, atomic_mass);
                        ...
                        for (int m3 = 0; m3 < N; m3++)
                        {
                            ...
                            for (int branch = 0; branch < 3; branch++)
                            {
                                ...
                                for (Axis ax : std::vector<Axis>{X, Y, Z})
                                {
                                    ...
                                    gammaM.coefficient += sqrt(ftN) / (2.0 * S) * (gammaMJxxmu - gammaMJyymu - i * gammaMJxymu - i * gammaMJyxmu) * sqrt(1 / (2 * atomic_mass * grid[m1][m2][m3].ph.E[branch])) * grid[m1][m2][m3].ph.polVectors[ax][branch];
                                    gammaP.coefficient += sqrt(ftN) / (2.0 * S) * (gammaPJxxmu - gammaPJyymu + i * gammaPJxymu + i * gammaPJyxmu) * sqrt(1 / (2 * atomic_mass * grid[m1][m2][m3].ph.E[branch])) * grid[m1][m2][m3].ph.polVectors[ax][branch];
                                    ...
                                    gammaZ.coefficient += sqrt(ftN) / (2.0 * S) * (2.0 * Jxxmu + 2.0 * Jyymu - 4.0 * Jzzmu + 2.0 * i * Jyxmu - 2.0 * i * Jxymu) * sqrt(1 / (2 * atomic_mass * grid[m1][m2][m3].ph.E[branch])) * grid[m1][m2][m3].ph.polVectors[ax][branch];
                                }
